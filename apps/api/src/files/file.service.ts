...
export class FileService {
  ...
  /**
   * Persist trusted, server-generated bytes (e.g. a rendered export) through
   * the same envelope path as {@link upload}, but WITHOUT the upload MIME/size
   * guards — those protect against hostile user input, which this is not. The
   * output is downloadable through the identical decrypt path.
   */
  async storeGenerated(
    ...
    await this.store.put(record.objectKey, encrypted.ciphertext);
    this.logger.info(
      { fileId: record.id, sizeBytes: content.length },
      'generated file stored encrypted',
    );
    return toView(record);
  }
